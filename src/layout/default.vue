...
const menuList = computed(() => {
  return [
    ...
    },
    {
      ...
      isActive: n => {
        return (
          route.params?.acct === userInfo.value?.acct && route.name === 'acct'
        )
      },
      mobileShow: () => false
    },
...
const mobileFooterList = computed(() => {
  const routerList = menuList.value.filter(item => {
    return (
      (item.mobileShow ? item.mobileShow() : true) &&
...
<template>
  <div
    class="layout"
    :class="{ hideHeader: !showMobileFooter }"
    v-swipe="handleSwipe"
  >
    <div class="mobile-header" :class="{ hide: !showMobileFooter }">
      <div class="nav-tab">
        ...
        <div class="btn-handle">
          <router-link
            class="nav-nuxt-link"
            style="font-size: 20px; color: rgb(var(--gray-6))"
            to="/search"
          >
          ...
          <template v-if="isLogin">
            <router-link
              ...
              to="/settings/profile"
            >
              <Icon icon="mingcute:settings-4-fill" />
            </router-link>
          </template>
          <template v-else>
            <a-button type="primary" @click="clickToLogin">{{
              t('login.login')
            }}</a-button>
          </template>
        </div>
          ...
          <div class="menu-routerlist">
            <template v-for="item in menuList">
              ...
              <router-link
                ...
                :class="{
                  'menu-panel_link-active': item.isActive
                    ? item.isActive(1)
              ...
              >
                <div class="menu-panel_link-icon">
                  ...
                  />
                  <a-badge
                    class="menu-panel_link-icon_badge"
                    :max-count="99"
                    :count="item.badge || 0"
                    style="--danger-6: var(--arcoblue-6)"
                  >
                  </a-badge>
                </div>
          ...
          <div class="menu-panel-userInfo">
            <template v-if="!userInfo?.acct">
              <div class="menu-panel-login">
                <a-button type="outline" @click="goRouter('/register')" long>
                  {{ t('login.createAccount') }}</a-button
                >
                <a-button type="primary" @click="clickToLogin">{{
      ...
      <div class="mobile-footer-top">
        ...
        <div class="mobile-handle-btn" :class="{ hide: !showMobileFooter }">
          ...
          <template v-if="showSendBtn">
            <router-link class="mobile-handle-send" v-if="isLogin" to="/send">
              <div class="icon">
    ...
    >
      <div class="mobile-slide">
        <div class="mobile-slide-header" v-if="isLogin">
          <router-link
            class="mobile-slide-header-avatar"
            :to="`/user/${userInfo.acct}`"
            @click="goTop"
          >
            <img :src="userInfo?.avatar || defaultAvatar" />
          </router-link>
          <router-link :to="`/user/${userInfo.acct}`" @click="goTop">
            <div class="mobile-slide-header-name">
          ...
          <div class="mobile-slide-header-follow-list">
            <router-link
              class="follow-item"
              :to="`/user/${userInfo.acct}?tab=tweets`"
              @click="goTop"
            >
              ...
              <template v-else>
                <div class="follow-item-value">
                  {{ userInfo?.status_count || 0 }}
                </div>
              </template>
            ...
            <router-link
              class="follow-item"
              :to="`/user/${userInfo.acct}/follower`"
              @click="goTop"
            >
              <template v-if="leftSildeLoading">
                <a-spin />
            ...
            <router-link
              class="follow-item"
              :to="`/user/${userInfo.acct}/following`"
              @click="goTop"
            >
              <template v-if="leftSildeLoading">
                <a-spin />
        ...
        <div class="mobile-slide-content">
          <template v-for="item in menuList">
            ...
            <router-link
              ...
              v-if="
                (item.isShow ? item.isShow() : true) &&
                drawerMenuList.includes(item.name)
              "
              :class="{
                'menu-panel_link-active': item.isActive
                  ? item.isActive(1)
        ...
        <AboutInstructions
          class="about-instructions"
          :showData="['github', 'version']"
        ></AboutInstructions>
      </div>
    </a-drawer>
  </div>
