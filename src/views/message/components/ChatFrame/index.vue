<script setup>
const { t } = useI18n()
import { getInfo, send, readDm } from '@/api/message.js'
...
const { data: chatUserInfo, isLoading: pending } = useQuery(
  ['chatUserInfo', chatId],
  () => getInfo(chatId)
)
...
onMounted(async () => {
  if (chatLog.value?.length) {
    ...
    notificationsNumStore.setNotificationsNum(notificationsRes)
  }
})
  ...
  <div class="chat-view">
    ...
    <a-spin class="input-frame" :loading="sendLoading">
      <a-input
        ...
        :placeholder="$t('message.createMessage')"
