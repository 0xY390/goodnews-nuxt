<script setup>
import { getBlockList, unBlockUser } from '@/api/account'
const { t } = useI18n()
const dataList = ref([])
...
const getData = async () => {
  ...
  const newList = Array.from(data).map(item => {
    return {
      ...item,
      _btnLoading: false
    }
  ...
  if (dataList.value.length >= Array.from(data).length) scrollbar.value = false
}
getData()
const delData = async item => {
  ...
  dataList.value = dataList.value.filter(
    i => i.target_account_id !== item.target_account_id
  )
}
</script>
      ...
      >
        <div v-for="item in dataList" class="item">
            ...
            <div class="user-info">
              ...
              <div class="flex items-center">
                <p class="user-name" @click="goRouter(item)">
                  {{
                    item.target_account?.display_name ||
                    item.target_account?.username
                  }}
                </p>
                <p class="user-acct" @click="goRouter(item)">
                  @{{ item.target_account?.acct }}
                </p>
              </div>
            </div>
          </UserCard>
          ...
          <div class="button-group">
            <a-button
              type="primary"
              :loading="item._btnLoading"
              @click="delData(item)"
            >
              {{ t('account.removeBlacklist') }}
            </a-button>
          </div>
        </div>
      </RealList>
