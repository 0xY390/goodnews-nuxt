...
const fetchData = async () => {
  const res = await getUserInfo(route.params.acct)
  hrefLink.value = res.account.url
}
...
<template>
  <div class="search-view">
    ...
    <section>
      ...
      <UserList :url="`/_api/v1/account/${route.params.acct}/${type}`">
      </UserList>
      <div class="follow-tips" v-if="hrefLink">
        <div class="line">{{ t('account.hideFollowers') }}</div>
        <div class="line">
          <a-link class="hrefLink" :href="hrefLink" target="_blank">{{
