<script setup>
import { getFilters, delFilters } from '@/api/filters.js'
...
const delFiltersAsync = async (id, index) => {
...
<template>
  <div class="admin-roles">
    <div class="header">
      ...
      <a-button type="primary" @click="addFilters">{{
        $t('filter.filterList.add')
      }}</a-button>
    </div>
      ...
      <div class="filter-card">
        ...
        >
          <div class="area-list-item" v-show="item?.kw_attr?.length">
            ...
            <div class="area-list-text">
              <span class="text-label"
                >{{ item?.kw_attr?.length }}
                {{ $t('filter.filterList.keyword') }}</span
              >
              <span class="text-value">{{
                item?.kw_attr?.map(item => item.kw).join(',')
              }}</span>
            </div>
          </div>
          <div class="area-list-item" v-show="item.status_count">
            ...
            <div class="area-list-text">
              <span class="text-label">{{
                t('filter.tweet', { count: item.status_count })
              }}</span>
              <span class="text-value">{{
                t('filter.tweet', { count: item.status_count })
              }}</span>
          ...
          <div class="filter-card-footer">
            ...
            <div class="actions">
              ...
              <a-button
                status="danger"
                @click="delFiltersAsync(item.id, index)"
                size="small"
              >
                <template #icon>
