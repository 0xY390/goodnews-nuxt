...
const columns = ref([
  ...
  },
  {
    ...
    render: ({ record: row }) => {
      const map = {
        0: t('common.already') + t('public.disable'),
        1: t('admin.relay.waitingRelayConfirm'),
        2: t('common.already') + t('public.enabled'),
  ...
  <div class="pages">
    ...
    <div class="main">
      <a-button
        long
        type="primary"
        @click="router.push('/settings/relayStation/new')"
        >{{ t('admin.relay.subscribeNewRelay') }}</a-button
      >
      <div class="list">
        <a-table
          ref="arcoTable"
        ...
        >
          <template #operation="{ record }">
            <template v-if="record.state === 2">
              <a-link size="small" @click="toggleState(record, false)">{{
                t('public.disabled')
              }}</a-link>
            </template>
            <template v-else-if="record.state === 0">
              <a-link size="small" @click="toggleState(record, true)">{{
                t('public.enabled')
              }}</a-link>
            </template>
            <a-popconfirm
              :content="t('admin.relay.deleteRelay')"
              type="warning"
