...
const rules = ref({
  ip: [
    {
      required: true,
      message: t('admin.ipRules.pleaseEnterIpAddress'),
      trigger: ['blur', 'change']
    }
  ],
  severity: [
    {
      required: true,
      message: t('admin.ipRules.pleaseSelectRule'),
      trigger: ['blur', 'change']
    }
  ]
})
...
<template>
  <div class="email-bomain-blocks-new">
    ...
    >
      ...
      <a-form-item
        :label="t('admin.ipRules.expirationTime')"
        field="expires_in"
      >
        <a-select v-model="ruleForm.expires_in">
          <a-option value="86400">{{
            t('admin.ipRules.theDay', { count: 1 })
          }}</a-option>
          <a-option value="1209600">{{
            t('admin.ipRules.theWeeks', { count: 2 })
          }}</a-option>
          <a-option value="2592000">{{
            t('admin.ipRules.theMonth', { count: 1 })
          }}</a-option>
          <a-option value="15552000">{{
            t('admin.ipRules.theMonth', { count: 6 })
          }}</a-option>
          <a-option value="31536000">{{
            t('admin.ipRules.theYear', { count: 1 })
          }}</a-option>
          <a-option value="94608000">{{
            t('admin.ipRules.theYear', { count: 3 })
          }}</a-option>
        </a-select>
        <template #extra>
        ...
        <a-radio-group v-model="ruleForm.severity">
          <a-radio value="1">
            <div class="severity-name">
              {{ t('admin.ipRules.limitRegistration') }}
            </div>
            <div class="severity-desc">
              {{ t('admin.ipRules.newRegistrationRequiresYourApproval') }}
            </div>
          </a-radio>
          <a-radio value="2">
            <div class="severity-name">
              {{ t('admin.ipRules.blockAccountRegistration') }}
            </div>
            <div class="severity-desc">
              {{ t('admin.ipRules.unableToRegisterNewAccount') }}
            </div>
          </a-radio>
          <a-radio value="3">
            <div class="severity-name">
              {{ t('admin.ipRules.blockAccess') }}
            </div>
            <div class="severity-desc">
              {{ t('admin.ipRules.blockAccessToAllResources') }}
            </div>
          </a-radio>
        </a-radio-group>
        <template #extra>
          <div class="desc">
            {{ t('admin.ipRules.chooseHowToHandleRequestsFromThisIp') }}
          </div>
        </template>
      </a-form-item>
