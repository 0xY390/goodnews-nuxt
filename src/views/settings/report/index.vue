      ...
      <a-form :model="form" layout="vertical">
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            <a-form-item
              field="source"
              :label="t('report.reportAccountSource')"
              label-col-flex="80px"
            >
        ...
        <a-row>
          <a-col :span="24">
            <a-form-item field="domain" label-col-flex="100px">
              <a-input
                v-model="form.domain"
                :placeholder="t('report.reportAccountDomain')"
              />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
          <a-col :span="12">
            <a-button long type="primary" @click="getData">{{
              t('common.search')
            }}</a-button>
          </a-col>
          <a-col :span="12">
            <a-button long type="primary" status="danger" @click="resetData">{{
              t('search.reset')
            }}</a-button>
          </a-col>
        </a-row>
      ...
      <div class="data-list">
        <div class="data-item" v-for="item in dataList" :key="item.id">
          <div class="data-user">
            ...
            <div class="info">
              <div class="total">
                {{ t('report.noteCount', { count: item.notes }) }}
              </div>
            </div>
          </div>
          <div
            class="reported-user"
            v-for="reportUser in item.reported"
            :key="reportUser.id"
          >
