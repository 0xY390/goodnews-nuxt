  ...
  try {
    ...
    panelData.value = [
      { total: res.account?.status_count, label: t('public.tweet') },
      {
        ...
        label: t('admin.account.media')
      },
      {
        total: res.account?.followers_count,
        label: t('admin.account.followers')
      },
      {
        total: res.target_report_count,
        label: t('admin.account.reportByAccount')
      },
      {
        total: res.report_count,
        label: t('admin.account.reportsForThisAccount')
      }
    ]
  } finally {
    loading.value = false
...
const warningActionMap = {
  1: t('account.frozen'),
  2: t('account.setSensitiveUser'),
  3: t('account.hideUser'),
  4: t('account.disableAccount')
}
    ...
    <a-spin :loading="loading">
      <div class="content">
        ...
        <div style="margin-bottom: 20px">
          <a-table size="small">
            <tbody class="table-body">
              ...
              <tr>
                <td>
                  {{ t('admin.account.email') + t('admin.account.address') }}
                </td>
              ...
              <tr>
                ...
                <td>
                  {{
                    pageData.account?.user?.confirmed_at
                      ? t('admin.account.confirmed')
                      : t('admin.account.unconfirmed')
                  }}
                </td>
                <td></td>
              </tr>
              ...
              <tr>
                <td>
                  {{
                    t('admin.account.lastActive', {
                      type: t('admin.ipRules.ip')
                    })
                  }}
                </td>
              ...
              <tr>
                <td>
                  {{
                    t('admin.account.lastActive', { type: t('public.time') })
                  }}
                </td>
        ...
        <a-space size="large">
          ...
          <a-button
            type="primary"
            :loading="actionMap['enableAccount'].loading"
            @click="handleAccountAction('enableAccount')"
            v-else
          >
          ...
          <a-button
            type="primary"
            :loading="actionMap['setAccountUnSensitive'].loading"
            @click="handleAccountAction('setAccountUnSensitive')"
            v-else
          >
          ...
          <a-button
            type="primary"
            :loading="actionMap['setAccountUnSilence'].loading"
            @click="handleAccountAction('setAccountUnSilence')"
            v-else
          >
          ...
          <a-button
            type="primary"
            :loading="actionMap['setAccountUnSuspend'].loading"
            @click="handleAccountAction('setAccountUnSuspend')"
            v-else
          >
        ...
        <a-row :style="{ width: '100%' }">
          <a-list :style="{ width: '100%' }">
            <a-list-item
              v-for="account_warning in pageData.account_warnings"
              :key="account_warning.id"
            >
              <a-list-item-meta
