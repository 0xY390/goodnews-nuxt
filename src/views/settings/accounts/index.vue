...
const status = ref(0)
const { roleId } = storeToRefs(transferStore)
setTimeout(() => {
  roleId.value = ''
}, 3000)
const sortord = ref(0)
...
const goDetail = rowData => {
  router.push(`/settings/accounts/detail/${rowData.account_id}`)
}
...
<template>
  <div class="admin-accounts">
    <div class="header">
      ...
      <div class="center">
        ...
        <a-input
          ...
          v-model="nickname"
          type="text"
          :placeholder="t('admin.account.nickname')"
        />
        <a-input
          class="center-input"
          v-model="email"
          type="text"
          :placeholder="t('admin.account.emailAddress')"
        />
        <a-input
          class="center-input"
          v-model="ip"
          type="text"
          :placeholder="t('admin.account.ipAddress')"
        />
      </div>
      <div class="bottom">
        <a-button class="search" @click="handleSearch" type="primary">
          {{ t('admin.account.search') }}
        </a-button>
        <a-button
          class="reset"
          @click="handleReset"
    ...
    >
      ...
      <a-link v-show="selectedAll" @click="selectedAll = false">{{
        t('admin.account.deselectAll')
      }}</a-link>
      <a-button
        type="primary"
        status="warning"
        @click="bannedUserAsync"
        size="small"
        >{{ t('admin.account.ban') }}</a-button
      >
    </div>
    <div class="table-wrapper">
      ...
      >
        <template #action="{ record }">
          <a-dropdown @click.stop>
            ...
            <template #content>
              <a-doption :value="1" @click="handleSelect(record, 1)">{{
                t('common.yes')
              }}</a-doption>
              <a-doption :value="0" @click="handleSelect(record, 0)">{{
                t('common.no')
              }}</a-doption>
            </template>
          </a-dropdown>
      ...
      >
        <template #prefix="{ itemCount }">
          {{ t('common.itemTotal', { total: itemCount }) }}
        </template>
      </a-pagination>
    </div>
  </div>
