...
const init = async () => {
  await userStore.updateUserInfo() // 更新用户信息
  initFields()
}
const initFields = () => {
  if (!!userInfo.value.fields) {
    // 循环4次，每次取当前索引的值，如果没有则设为默认值
    const defaultObj = {
      name: '',
      type: 'PropertyValue',
      value: ''
    ...
    for (let index = 0; index < 4; index++) {
      if (index in userInfo.value.fields) {
        defaultFields.value[index] = userInfo.value.fields[index]
      } else {
        defaultFields.value[index] = cloneFnJSON(defaultObj)
      }
    }
  }
}
...
const formValue = reactive({
  display_name: userInfo.value.display_name || userInfo.value.username,
  note: userInfo.value.note,
  avatar_attachment_id: '',
  profile_image_attachment_id: '',
  manually_approves_follower: userInfo.value.manually_approves_follower === 1,
  is_bot: userInfo.value.is_bot,
  is_sensitive:
    userInfo.value.sensitized_at || userInfo.value.is_sensitive === 1,
  is_display_sensitive: userInfo.value.is_display_sensitive === 1,
...
const onSubmit = async () => {
  ...
  try {
    ...
    cacheDataStore.updateAccountInfo({})
  } catch (err) {
    submitLoading.value = false
...
<template>
  <div class="content">
    ...
    <a-form :model="formValue" class="a-form" layout="vertical">
      ...
      <a-form-item>
        <a-spin class="form-item-file" :loading="uploadLoading1">
          <div class="form-item">
            ...
            <input
              class="form-item-file input-file"
              type="file"
      ...
      <a-form-item>
        <div class="form-item">
          <a-checkbox v-model="formValue.manually_approves_follower">{{
            t('profile.protectAccount')
          }}</a-checkbox>
          <div class="item-desc-min">
            {{ t('profile.youNeedToManuallyReviewAllFollowRequests') }}
          </div>
        </div>
      </a-form-item>
      <a-form-item>
        <div class="form-item">
          <a-checkbox v-model="formValue.is_bot">{{
            t('profile.thisIsARobotAccount')
          }}</a-checkbox>
          <div class="item-desc-min">
            {{
              t(
                'profile.mostOperationsFromThisAccountAreAutomatedAndMayNotBeMonitored'
              )
            }}
          </div>
        </div>
      ...
      <a-form-item>
        <div class="form-item">
          <a-checkbox
            ...
            :title="
              sensitiveDisabled &&
              t('profile.yourAccountHasBeenSetAsSensitiveByTheAdministrator')
            "
          >
            {{
              t(
                'profile.markYourMediaContentAsPotentiallyContainingSensitiveContent'
              )
            }}
          </a-checkbox>
          <div class="item-desc-min">
            {{
              t(
                'profile.ifEnabledImagesAndVideosYouPostWillBeMarkedAsSensitiveForPeopleWhoDoNotWantToSeeSensitiveContent'
              )
            }}
          </div>
        </div>
      ...
      <a-form-item>
        <div class="form-item">
          <a-checkbox v-model="formValue.is_display_sensitive">{{
            t('profile.displayMediaContentThatMayContainSensitiveContent')
          }}</a-checkbox>
        </div>
      </a-form-item>
        ...
        <div class="form-item">
          <div class="addition">
            ...
            <div class="item-desc-min">
              {{
                t(
                  'profile.thisWillBeDisplayedOnYourProfilePageInTableFormUpToItems',
                  { count: 4 }
                )
              }}
            </div>
            ...
            <div class="form-item-addition">
              <a-space v-for="item in formValue.fields" class="a-space">
                <a-input
                  class="a-input"
                  :placeholder="t('public.tag')"
                  v-model="item.name"
                  allow-clear
                />
                <a-input
                  class="a-input"
                  :placeholder="t('public.content')"
                  v-model="item.value"
                  allow-clear
                />
              </a-space>
      ...
      <a-form-item>
        <a-button
          :disabled="uploadLoading1 || uploadLoading2"
          type="primary"
          @click="onSubmit"
          block
          :loading="submitLoading"
          >{{ $t('profile.save') }}</a-button
        >
      </a-form-item>
    </a-form>
  </div>
