...
const languageList = langList.map(item => ({
  label: t(item.label),
  value: item.value
}))
...
const fetchData = async () => {
  ...
  dataList.value = data.filter(item => item.account) || []
      ...
      <a-form :model="filterValue" layout="vertical">
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item
              field="value1"
              :label="t('public.selectLanguage')"
              label-col-flex="100px"
            >
              <a-select
                v-model="filterValue.language"
                :style="{ width: '320px' }"
