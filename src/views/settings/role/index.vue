...
<template>
  <div class="admin-roles">
    <div class="header">
      ...
      <a-button class="btn" @click="addRole">{{
        t('admin.role.addRole')
      }}</a-button>
    </div>
    <div class="desc">
      {{ t('admin.role.use') }}
      <span class="font-bold">{{ t('admin.role.userRole') }}</span>
    ...
    <div class="default">
      <div class="default-top">
        ...
        <span style="margin-left: 4px">{{
          t('admin.role.defaultPermissions')
        }}</span>
      </div>
      <div class="flex justify-between items-center">
        <span style="flex: 1">{{
          t(
            'admin.role.thisIsTheBasicRoleThatAffectsAllUsersIncludingThoseWhoAreNotAssignedARole'
          )
        }}</span>
      </div>
    </div>
          ...
          <div class="item-center">
            <div class="item-center-left">
              ...
              <span
                ...
                class="cursor-pointer"
                >{{ item.priority }} {{ t('admin.role.permissions') }}</span
              >
            </div>
            <div
              class="item-center-right"
              @click="editRole(item.id, item.name)"
            >
              <Icon icon="mdi:pencil" /><span
                class="ml-1 font-bold cursor-pointer"
                >{{ t('common.edit') }}</span
              >
            </div>
          </div>
