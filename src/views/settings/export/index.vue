    ...
    render: ({ record }) => {
      if (record.link) {
        return h(
          ...
          {
            style:
              'display: flex; align-items: center; gap: 4px; --primary-6: var(--gray-8)',
            type: 'text',
            onClick: async () => {
          ...
          () => [h(Icon, { icon: 'ph:download-fill' }), h('span', 'CSV')]
        )
      }
    }
  }
    ...
    render: ({ record }) => {
      if (record.filesize) {
        return h(
          ...
          () => [
            h(Icon, { icon: 'ph:download-fill' }),
            h('span', t('admin.importExport.downloadYourArchive'))
          ]
        )
      } else {
      }
  ...
  <div class="email-bomain-blocks">
    ...
    <div class="table-wrapper">
      ...
      <div>
        ...
        >
          {{ t('admin.importExport.requestYourArchive') }}
        </a-button>
      </div>
