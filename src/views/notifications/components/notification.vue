...
<template>
  <div>
      ...
      <div class="notification-item">
        <div class="header-item">
          ...
          <UserCard :account="tweetData.account">
            <span
              class="text-under text-bold user-acct"
              @click="goRouter(`/user/${tweetData.account.acct}`)"
            >
      ...
      <div class="notification-item">
        <div class="header-item">
          <Icon
            class="header-item-icon"
            name="ci:mention"
            style="color: rgb(var(--arcoblue-6)); font-size: 20px"
          />
    ...
    <template v-else-if="tweetData.notify_type === 3">
      <div class="notification-item">
        <div class="header-item">
          ...
          <span>{{
            tweetData?.status?.reply_to_id
              ? t('notification.retweeted') + t('notification.reply')
              : t('notification.retweeted') + t('notification.tweet')
          }}</span>
        </div>
        <TweetMessage
          :status="tweetData?.status"
      ...
      <div class="notification-item">
        <div class="header-item">
          ...
          <span>
            {{
              tweetData?.status?.reply_to_id
                ? t('notification.liked') + t('notification.reply')
                : t('notification.liked') + t('notification.tweet')
            }}
          </span>
        </div>
        <TweetMessage
    ...
    <template v-else-if="tweetData.notify_type === 9">
      <div class="notification-item">
        <div class="header-item">
          <SvgIcon class="header-item-icon" name="vote"></SvgIcon>
    ...
    <template v-else-if="tweetData.notify_type === 11">
      <div class="notification-item">
        <div class="header-item">
          <Icon
            class="header-item-icon"
            name="heroicons:user-plus-20-solid"
            style="color: rgb(var(--arcoblue-6))"
          />
          <UserCard :account="tweetData.account">
            <span
            ...
            <span class="mla">{{
              $dayjs(tweetData.created_at).format('MM-DD')
            }}</span>
          </UserCard>
        </div>
        <UserItem :data="tweetData.account" :isShowFollow="false"></UserItem>
    ...
    <template v-else-if="tweetData.notify_type === 20">
      <div class="notification-item">
        <div class="header-item">
          <SvgIcon name="arrow-up-circle" style="color: #ff9500"></SvgIcon>
          <span class="text-bold">{{
            $t('notification.newVersionNotify')
          }}</span>
        </div>
        <div class="content">
          <div
            class="desc"
            @click="openUrl(tweetData.status)"
            v-html="tweetData.status.content"
          ></div>
        </div>
      </div>
    </template>
