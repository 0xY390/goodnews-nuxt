    ...
    <a-spin class="content" :loading="filterLoading">
      <template v-if="!isFiltered">
        ...
        <div class="search">
          <a-input-search
            v-model="searchVal"
            :placeholder="t('tweet.filter.searchOrCreateFilter')"
          >
          </a-input-search>
        </div>
        ...
        <div class="list list2" v-if="searchVal">
          <div
            class="item"
            v-for="item in searchFilterList"
            :key="item.id"
            @click="handleFilter(item)"
          >
            {{ item.title }}
          </div>
          <div class="item" @click="handleCreateFilter(searchVal)">
            + {{ t('tweet.filter.newFilter') }} {{ searchVal }}
          </div>
        </div>
      </template>
      <template v-else>
        ...
        <div class="subtitle">
          {{ t('tweet.filter.thisTweetAdded', { filterName: searchVal }) }}
        </div>
        <a-button
          class="success-btn"
          type="primary"
