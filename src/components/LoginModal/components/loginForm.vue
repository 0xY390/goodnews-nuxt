...
const handleSubmit = async formData => {
  ...
  try {
    // 请求token
    const userData = await userStore.login({ email, password }).finally(() => {
  ...
  <a-form :model="formValue" @submit="handleSubmit" layout="vertical">
    ...
    >
      <a-input
        ...
        :placeholder="t('login.pleaseInputByEmail')"
      />
    </a-form-item>
    <a-form-item
      field="password"
    ...
    <a-button html-type="submit" long type="primary" :loading="loginLoading">{{
      t('login.login')
    }}</a-button>
  </a-form>
  <div class="noAccount">
