...
import SelectByBtn from '@/components/SelectByBtn/index.vue'
...
const sendTweetDisabledReason = computed(() => {
  ...
  const validContent = tweetContentText.value
    ...
    .replace(/ /g, '')
    .replace(/<\/p>/g, '')
  if (!validContent.length && !fileList.value.length)
    reason = t('tweet.noContent')
  return reason
})
...
async function addNewTweet() {
  ...
  useMessage().success(
    tweetEditData.value
      ? t('common.modifySuccessfully')
      : t('tweet.sendTweet') + t('common.success')
  )
  content.value = ''
  fileList.value = []
  closeVote()
...
<template>
  <form v-on:submit.prevent="submitForm" class="editor-form">
    <div class="tweetModeEdit">
      <a-tag closable v-if="tweetEditData" @close="setTweetForm">
        {{ t('common.edit') }}
      </a-tag>
    </div>
    ...
    <div class="btn-frame" style="--btn-font-size: 18px">
      ...
      <a-tooltip :content="t('tweet.addVote')" mini position="top">
        <a-button
          type="text"
          @click="openVote"
          style="font-size: var(--btn-font-size)"
        >
          <SvgIcon name="vote"></SvgIcon>
        </a-button>
      </a-tooltip>
      <SelectByBtn
        v-model="pushSite"
        :options="pushSiteOptions"
        :title="t('tweet.isPush')"
        :tips="t('tweet.choosePush')"
      >
        <Icon icon="material-symbols:webhook-rounded" />
      </SelectByBtn>
