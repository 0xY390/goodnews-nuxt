...
const searchTypeList = [
  {
    label: t('search.match') + ' ${title} ' + t('search.tweets'),
    value: 'status'
  },
  {
    label: t('search.match') + ' ${title} ' + t('search.hashtags'),
    value: 'hashtag'
  },
  {
    label: t('search.match') + ' ${title} ' + t('search.accounts'),
    value: 'account'
  }
]
  ...
  return list
    .map(item => {
      return item === keyword.value
        ? `<span style="color: rgb(var(--arcoblue-6))">${item}</span>`
        : item
    })
    .join('')
}
        ...
        <div class="container">
          ...
          <template v-else>
            <div class="list">
              <div class="type" v-if="historyList.length">
                ...
                >
                  ...
                  <div class="text-box">
                    ...
                    <a-tag class="type" v-if="item.type">{{
              ...
              <div
                v-else
                style="height: 100px; text-align: center; padding-top: 20px"
              >
                {{ t('search.trySearch') }}
              </div>
            </div>
