...
import {
  setAccountSensitiveUser,
  setAccountUnSensitiveUser
  ...
  <div class="icon-more">
    <a-dropdown @select="handleCommand" :popup-max-height="false" position="br">
      <a-button shape="circle" type="text">
        <Icon icon="akar-icons:more-horizontal-fill"></Icon>
      </a-button>
      <template #content>
        <a-doption value="delete" v-if="props.options.showDelete">
          <div class="dropdown-item red" v-if="isMyTweet">
            <Icon class="icon-box" icon="material-symbols:delete"></Icon>
            {{ t('common.delete') }}
          </div>
        </a-doption>
        <a-doption
          value="openOriginal"
          v-if="props.options.openOriginUrl && props.status.url"
        >
          <div class="dropdown-item">
            <Icon class="icon-box" icon="wpf:connected"></Icon>
            {{ t('tweet.openOriginPage') }}
          </div>
        </a-doption>
        <a-doption value="embed" v-if="props.options.embed">
          <div class="dropdown-item">
            <Icon class="icon-box" icon="mingcute:code-line"></Icon>
            {{ t('tweet.embedPost') }}
          </div>
        </a-doption>
        <a-doption value="mention" v-if="props.options.mention">
          <div class="dropdown-item">
            <Icon class="icon-box" icon="fluent:mention-16-filled"></Icon>
            {{ t('tweet.mention') }}
          </div>
        </a-doption>
        ...
        <a-doption value="message" v-if="props.options.message && !isMyTweet">
          <div class="dropdown-item">
            <Icon class="icon-box" icon="uil:envelope-alt"></Icon>
        ...
        <template v-if="props.options.follow">
          <a-doption
            value="follow"
            v-if="!props.status.account.follower && !isMyTweet"
          >
            <div class="dropdown-item">
              <SvgIcon class="icon-box" name="guanzhu"></SvgIcon>
              {{ `${t('account.following')} ${props.status.account.acct}` }}
            </div>
          </a-doption>
          <a-doption value="unfollow" v-else-if="props.status.account.follower">
            <div class="dropdown-item">
              <SvgIcon class="icon-box" name="guanzhu-2"></SvgIcon>
        ...
        <template v-if="props.options.top && isMyTweet">
          <a-doption value="pin" v-if="!props.status.pinned_at">
            <div class="dropdown-item">
              <Icon class="icon-box" icon="fluent:stack-star-16-filled"></Icon>
              {{ t('tweet.pin') }}
            </div>
          </a-doption>
          ...
          <a-doption value="unpin" v-else>
            <div class="dropdown-item">
              <Icon
                class="icon-box"
                icon="fluent:set-top-stack-16-regular"
              ></Icon>
              {{ t('tweet.unpin') }}
            </div>
          </a-doption>
        ...
        <a-doption
          ...
          v-if="
            props.options.hide &&
            !isMyTweet &&
            props.status.reply_to_account_id === myUserId
          "
        >
          <div class="dropdown-item">
            <SvgIcon class="icon-box" name="hide"></SvgIcon>
        ...
        <a-doption
          value="filterPost"
          v-if="props.options.filterPost && props.status.filter === null"
        >
          <div class="dropdown-item">
            <SvgIcon class="icon-box" name="hide"></SvgIcon>
            {{ `${t('tweet.filterPost')}` }}
        ...
        <a-doption value="report" v-if="!isMyTweet">
          <div class="dropdown-item">
            <Icon class="icon-box" icon="ic:outline-report" />
        ...
        <template v-if="isAdminUser">
          ...
          <a-doption value="openUserByAdmin">
            <div class="dropdown-item">
              <Icon class="icon-box" icon="clarity:administrator-line" />
              {{
          ...
          <a-doption
            value="openSiteByAdmin"
            v-if="props.status.account?.domain"
          >
            <div class="dropdown-item">
              <Icon class="icon-box" icon="mingcute:sitemap-line" />
          ...
          <template v-if="!props.status.account.sensitized_at">
            <a-doption value="markSensitiveAccount">
              <div class="dropdown-item">
                <Icon class="icon-box" icon="ion:warning-outline" />
                {{
    ...
    <delete-tweet-pop
      v-model:visible="isShowDeletePop"
      @confirm="deleteTweet"
      :loading="deteleLoading"
    ></delete-tweet-pop>

    <FilterModal
      v-if="props.status.id && showFilterModal"
      :status="props.status"
