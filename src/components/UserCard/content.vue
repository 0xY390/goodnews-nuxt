  ...
  <div class="container">
    ...
    <div class="body">
      ...
      <div class="follow-block">
        <div
          class="follow-item"
          @click.stop="goPath(`/user/${props.account.acct}?tab=tweets`)"
        >
          <div class="item-count">
            {{ props.account.status_count || 0 }} {{ $t('public.tweet') }}
          </div>
        </div>
        <div class="follow-item" @click.stop="goUserSocial('following')">
          <div class="item-count">
            {{ props.account.following_count || 0 }}
            {{ $t('account.following') }}
          </div>
        </div>
        <div class="follow-item" @click.stop="goUserSocial('follower')">
          <div class="item-count">
            {{ props.account.followers_count || 0 }}
            {{ $t('account.follower') }}
          </div>
        </div>
      </div>
